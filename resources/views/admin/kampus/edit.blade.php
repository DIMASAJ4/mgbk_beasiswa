    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-3xl p-8 relative overflow-hidden border border-slate-100 shadow-xl shadow-slate-100/10">
            ...
            <div class="relative z-10 mb-8 border-b border-slate-100 pb-6">
                <h2 class="heading-font text-2xl font-extrabold text-[#1a3d6e] tracking-tight">Edit Kampus Mitra</h2>
                <p class="text-slate-500 text-xs mt-1.5">Perbarui rincian perguruan tinggi mitra penyedia beasiswa.</p>
            </div>

            <form action="{{ route('admin.kampus.update', $kampus->id) }}" method="POST" enctype="multipart/form-data" class="relative z-10 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="nama_kampus" name="nama_kampus" required value="{{ old('nama_kampus', $kampus->nama_kampus) }}" placeholder="Contoh: Universitas Gadjah Mada"
                    ...
                    <div>
                        ...
                        <input type="url" id="website" name="website" value="{{ old('website', $kampus->website) }}" placeholder="Contoh: https://ugm.ac.id"
                               class="w-full px-4 py-3 bg-white border border-slate-200 focus:border-[#1D9E75] focus:ring-1 focus:ring-[#1D9E75] rounded-xl text-xs text-slate-700 placeholder-slate-400 transition-all shadow-sm">
                    ...
                    <div>
                        ...
                        <input type="text" id="kontak" name="kontak" required value="{{ old('kontak', $kampus->kontak) }}" placeholder="Contoh: Humas (0812-3456-7890)"
                               class="w-full px-4 py-3 bg-white border border-slate-200 focus:border-[#1D9E75] focus:ring-1 focus:ring-[#1D9E75] rounded-xl text-xs text-slate-700 placeholder-slate-400 transition-all shadow-sm">
                    ...
                    <div>
                        ...
                        @if ($kampus->logo)
                            <div class="mb-2 flex items-center gap-3 p-2 bg-slate-50 border border-slate-200 rounded-lg max-w-sm">
                                <img src="{{ asset('storage/' . $kampus->logo) }}" class="h-10 w-10 object-cover rounded border border-slate-200" alt="">
                                <span class="text-[10px] text-slate-450 truncate block">{{ basename($kampus->logo) }}</span>
                            </div>
                        @endif
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="alamat" name="alamat" rows="2" required placeholder="Tuliskan alamat lengkap kampus..."
                                  class="w-full px-4 py-3 bg-white border border-slate-200 focus:border-[#1D9E75] focus:ring-1 focus:ring-[#1D9E75] rounded-xl text-xs text-slate-700 placeholder-slate-400 transition-all shadow-sm">{{ old('alamat', $kampus->alamat) }}</textarea>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4" required placeholder="Tuliskan keunggulan kampus..."
                                  class="w-full px-4 py-3 bg-white border border-slate-200 focus:border-[#1D9E75] focus:ring-1 focus:ring-[#1D9E75] rounded-xl text-xs text-slate-700 placeholder-slate-400 transition-all shadow-sm">{{ old('deskripsi', $kampus->deskripsi) }}</textarea>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <label class="relative flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-200 hover:border-[#1D9E75] cursor-pointer select-none transition-all group shadow-sm">
                            <div class="flex items-center gap-3">
                                <input type="radio" name="is_active" value="1" {{ old('is_active', $kampus->is_active) ? 'checked' : '' }} class="h-4.5 w-4.5 text-[#1D9E75] focus:ring-[#1D9E75] focus:ring-offset-0 border-slate-350 bg-white cursor-pointer">
                                <div class="text-left">
                        ...
                        <label class="relative flex items-center justify-between p-4 rounded-2xl bg-white border border-slate-200 hover:border-rose-500 cursor-pointer select-none transition-all group shadow-sm">
                            <div class="flex items-center gap-3">
                                <input type="radio" name="is_active" value="0" {{ !old('is_active', $kampus->is_active) ? 'checked' : '' }} class="h-4.5 w-4.5 text-rose-600 focus:ring-rose-500 focus:ring-offset-0 border-slate-350 bg-white cursor-pointer">
                                <div class="text-left">
                ...
                <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
                    ...
                    <button type="submit" class="px-6 py-3 rounded-xl bg-[#1D9E75] hover:bg-[#15825f] text-white text-xs font-bold transition-all shadow-lg shadow-[#1D9E75]/10 cursor-pointer">
                        <i class="fa-solid fa-cloud-arrow-up mr-1.5"></i> Perbarui Mitra
                    </button>
                </div>
            </form>
        </div>
